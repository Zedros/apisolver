...
function createInstanceFile(string,filename) {
   ...
   console.log('CARPETA CREADA PARA INS HEHE :', filename);
...
router.post("/load", function (req, res, next) {
    ...
    shell(comando);
    setTimeout(() => {
        file.mv('../temp/'+ filename +'/save.tar', function (err, result) {
            if (err)
                throw err;
            file.name = "save";
            ...
            setTimeout(() => {
                fs.readdir('../temp/'+ filename, function (err, files) {
                    ...
                    ress = crearInstanciaWithFile(filename,false,loadFile,saveFile);
                    setTimeout(() => {
                        shell('cd .. ; cd temp ; cd '+ filename +'; rm '+saveFile+' ; mv '+loadFile+' load'+filename+'ins.txt');
                    }, 1300);
                });
                res.send({
                    success: true,
...
router.post('/seleccionarZona', (req, res, next) => {
    ...
    if (req.body.tipo == 'zoom_in') {
        ...
        console.log(ress);
        res.status(200).json({
            message: 'Command received : Zoom in',
            respuesta: ress
        });
    }
    if (req.body.tipo == 'zoom_out') {
        ...
        console.log(ress);
        res.status(200).json({
            message: 'Command received : Zoom Out',
            respuesta: ress
        });
    }
...
router.post('/getSolution', (req, res, next) => {
    ...
    if (req.body.x) {
        ...
        res.status(200).json({
            message: 'Command received : Get solution',
            respuesta: ress
        });
    }
    res.status(200).json({
...
router.post('/focus', (req, res, next) => {
    ...
    if (req.body.x) {
        ...
        res.status(200).json({
            message: 'Command received : RPM '+req.body.x+';'+req.body.y,
            respuesta: ress
        });
    }
    else{
...
router.post('/comando', (req, res, next) => {
    ...
    ress = shell(comando);
    console.log(ress);
    if (string.substring(0,4) == 'save') {
        res.status(200).json({
            message: 'Command received : save instance ',
            respuesta: ress
        });
    }
    res.status(200).json({
        message: 'Command received : ' + value,
        respuesta: ress
    });
});
